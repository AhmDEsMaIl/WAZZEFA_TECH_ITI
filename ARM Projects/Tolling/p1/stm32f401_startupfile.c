...
// vector table  -> array of address{msp,rst,nmi,ext0}
...
void Reset_Handler(void) 
{
	// realocate from flash to ram 
	// calculation of size of .data section 
	u32 data_size=(u32) (&e_data) - (u32) (&s_data);
	u32 *Data_ScrcPtr=(&(e_text)+1);
	u32 *DataDesAddress=(&s_data);
	for(u32 i=0;i<data_size;i++)
	{
		*DataDesAddress=*Data_ScrcPtr;
		DataDesAddress++;
		Data_ScrcPtr++;
	}
	// INIT .bss with zeros
	...
	// calling main
	main();
	// exit when there no while(1) in main
	//exit routine
	while(1);
}
